...
class Eagle3Model(nn.Module):
    ...
    def _gather_parent_top2_raw_logits(
        ...
        ``headout`` is ``(num_parents, draft_vocab_size)``; ``topk_index`` is the
        caller's log-softmax top-k indices. Log-softmax is monotone per row, so
        gathering ``headout`` at the first two index columns equals a fresh
        vocab-wide top-2 (tied values may swap order; the downstream ratio is
        symmetric under z1 == z2). Returns shape ``(num_parents * top_k, 2)`` so
        entries concatenate along dim 0 in the same parent-major order as the
        flattened ``logprobs_list``.
        """
        top_k = int(topk_index.shape[-1])
        k = min(2, int(headout.shape[-1]))
        if top_k >= k:
            top2_values = torch.gather(headout, -1, topk_index[..., :k])
        else:
