...
> 创建日期：2026-05-23
> 最近更新：2026-06-01（feature `2026-06-01-sam-eagle3-tree-fusion` 归并）
...
- **`samd/`** — SAM-Decoding 主实现，含 SAM 切换 + 可插拔 tree_model
  - **`samd/tree_model/`** — Draft 模型集成层，按 `samd_config.tree_method` 路由到具体实现：
    ...
    - `fusion.py` — 候选树融合工具（2026-06-01 引入）。当前支持 `TreeSpec(tokens, parents)` 内部表示、从 EAGLE3 tree buffers 反推树结构、导出 `tree_attn_mask / tree_position_ids / tree_retrieve_indices`，以及把 raw SAM sequence 按 shared prefix graft 到 EAGLE3 tree 上。融合层位于 DraftModel / tree_model 输出组合层，不侵入 EAGLE3 draft model 搜索逻辑
  - **SAM + tree_model 候选编排** — 默认 `tree_fusion="none"` 时沿用原来的 SAM sequence 与 tree model 二选一路径；当 `tree_method="eagle3"` 且 `tree_fusion="sam_sequence_graft"` 时，`DraftModel.lookup` 会先生成完整 EAGLE3 tree，再在 SAM match 达阈值时把不含 padding 的 raw SAM sequence graft 成额外分支，最终仍以标准 tree candidate 交给 `SamdModel.decode` / `eval_posterior` 验证
...
- **`tree_method` 与 `tree_fusion` 分别表示不同维度**：`tree_method` 选择 draft tree model 类型（如 `eagle3`），`tree_fusion` 选择候选树组合策略（当前 `none` / `sam_sequence_graft`）。`tree_fusion="sam_sequence_graft"` 仅支持 `tree_method="eagle3"`，其他组合初始化时直接报错；不要新增 `tree_method="eagle3_fusion"` 这类混合枚举
